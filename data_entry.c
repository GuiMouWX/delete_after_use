...
)
{
  ...
  DBG_POINT;
	...
	{
	DBG_POINT;
		res = screen_kbd_touch_read_with_abort( &key_code, timeout, VIM_FALSE, VIM_FALSE );
	DBG_POINT;
	}
	else
	{
		res = vim_screen_kbd_or_touch_read( &key_code, timeout, VIM_FALSE );
	}
    ...
    else if ((key_code == VIM_KBD_CODE_CANCEL) && (!(options->entry_options & KBD_OPT_PASSWORD)))
    {
      ...
      // 211119 MB says make it alwys clear the data - ie get rid of the else (below)
		//else
...
#if 0
/*----------------------------------------------------------------------------*/
VIM_ERROR_PTR vim_kbd_sound(void)
{
  VIM_ERROR_RETURN_ON_FAIL(vim_sound_tone(VIM_KBD_DEFAULT_FREQUENCY_VALID_KEYPRESS,VIM_KBD_DEFAULT_DURATION_VALID_KEYPRESS));
  return VIM_ERROR_NONE;
}
/*----------------------------------------------------------------------------*/
VIM_ERROR_PTR vim_kbd_invalid_key_sound(void)
{
  VIM_ERROR_RETURN_ON_FAIL(vim_sound_tone(VIM_KBD_DEFAULT_FREQUENCY_INVALID_KEYPRESS,VIM_KBD_DEFAULT_DURATION_INVALID_KEYPRESS));
  return VIM_ERROR_NONE;
}
#endif
/*----------------------------------------------------------------------------*/
...
VIM_ERROR_PTR data_entry_confirm( VIM_MILLISECONDS timeout, VIM_BOOL fAllowCancel, VIM_BOOL fCheckCardRemoved )
{
  ...
  {		  
	...
	{
		res = vim_screen_kbd_or_touch_read( &key_pressed, timeout, VIM_FALSE );
	}
...
/*----------------------------------------------------------------------------*/
#if 1
VIM_ERROR_PTR GetSettlementDate( void )
{  
	...
	entry_opts.entry_options = KBD_OPT_ALLOW_BLANK| KBD_OPT_SETTLE;
